...
This one is subjective and there's no best answer. Come up with your own "I
before E" rule. Your rule must:

    depend on the ordering of the letters I and E when they appear next to
    each other. That is, if a word contains an I and an E next to each other,
    and it follows your rule, then when you swap those two letters, the new
    word must not follow your rule.

    depend only on the spelling of a word, not its pronunciation or meaning.

    be simple enough that schoolchildren can apply it.

For instance, I just came up with a rule "I before E, except when followed by
G". This rule has 1,544 exceptions in the enable1 word list. How many
exceptions does your rule have?
'''

def check(word):
    good = True
    if (any(['cie' in word, 'cei' not in word and 'ei' in word])):
        good = False
    print (good)
    return good
check("transceiver")
